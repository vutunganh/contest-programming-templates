...
struct SuffixArray {
  ...
  // O(L log^2 L)
  SuffixArray(const string &s) : L(s.size()), s(s), P(1, vector<ll>(L, 0)), M(L) {
  ...
  vector<ll> GetSuffixArray() { return P.back(); }
  ...
  // starts of each suffix increasingly ordered
  vector<ll> OrderedSuffixes() {
    vector<ll> lca = GetSuffixArray();
    vector<ll> r(lca.size());
    F(lca.size()) r[ lca[i] ] = i;
    return r;
  }
};
